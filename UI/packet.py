...
class DataPacket:
    ...
    def __init__(self, header=0xAC, sequence=0, timestamp=0,
                 channel0=0.0, channel1=0.0, internal_adc=0, battery_voltage=0.0, crc=0):
        self.header = header
    ...
    def get_all_data(self) -> tuple[int, int, int, float, float, int, float, int]:
        return (self.header, self.sequence, self.timestamp,
                self.channel0, self.channel1, self.internal_adc, self.battery_voltage, self.crc)
    ...
    def set_battery_voltage(self, battery_voltage: float):
        self.battery_voltage = float(battery_voltage)
    ...
    def to_display_string(self) -> str:
        return (f"0x{self.header:02X} | {self.sequence:3d} | "
                ...
                f"{self.channel1:7d} | {self.internal_adc:5d} | {self.battery_voltage:7.3f} | "
                f"{self.crc:4d}")
